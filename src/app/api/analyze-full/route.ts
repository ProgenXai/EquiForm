...
const FRONT_VIEW_VALIDATION_PROMPT =
  "Does this image show a single horse in a correct front view for conformation analysis? To pass ALL of these must be true: the horse is facing directly toward the camera (not angled), the horse is standing completely still with all four feet flat on the ground, both front legs are visible (partial visibility due to lighting or dark coat color is acceptable), the chest and shoulders are square to the camera, and no people, ropes, or objects are blocking the legs or body. Reject if the horse is clearly angled, walking, trotting, or if a lead rope is pulling the head severely to one side. Do not reject for dark coat color.";
