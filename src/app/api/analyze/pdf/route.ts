...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (model3dSnapshotImage) {
      ...
      const overallText = `Overall Score: ${report.overall_score}/100`;
      drawCenteredText(page, overallText, y, fontBold, 14, ACCENT_RGB);
      y -= 28;

      const closingFooterY = MARGIN + 24;
      drawCenteredText(
        page,
      ...
      const snapshotAreaBottom = closingFooterY + 18;
      const availableSnapshotHeight = y - snapshotAreaBottom;
      const snapshotScale = Math.min(
        CONTENT_WIDTH / model3dSnapshotImage.width,
        availableSnapshotHeight / model3dSnapshotImage.height,
      );
      ...
      const snapshotY =
        snapshotAreaBottom + (availableSnapshotHeight - snapshotHeight) / 2;
